...
describe("SecureAssetPlatform", function () {
  ...
  it("enables secure two-step default administrator rotation", async function () {
    const { platform, admin, replacement } = await deployFixture();
    await expect(platform.connect(admin).beginDefaultAdminTransfer(replacement.address))
      .to.emit(platform, "DefaultAdminTransferScheduled");
    await platform.connect(replacement).acceptDefaultAdminTransfer();
    expect(await platform.hasRole(DEFAULT_ADMIN_ROLE, replacement.address)).to.equal(true);
    expect(await platform.hasRole(DEFAULT_ADMIN_ROLE, admin.address)).to.equal(false);
    expect(await platform.defaultAdmin()).to.equal(replacement.address);
  });
