...
describe("audit API boundary", () => {
    ...
    it("accepts only the sanitized contract and omits browser credentials", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://api.example.invalid/v1/audit?limit=10&projection_status=canonical", expect.objectContaining({ credentials: "omit" }));
    });
});
