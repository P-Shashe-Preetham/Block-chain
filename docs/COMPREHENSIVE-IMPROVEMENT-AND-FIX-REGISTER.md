...
| SC-ENG-02 | Add Slither or an equivalent Solidity static-analysis gate and publish reviewed findings. | P0 | Partial: pinned Slither 0.11.6 workflow added; local production-surface run found only low timestamp-use findings; remote SARIF review pending | CI report, approved suppressions with rationale, zero unreviewed high findings. |
...
| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| API-01 | Implement the FastAPI service or remove aspirational API commands until implementation exists. A fail-closed boundary now exists with versioned OpenAPI metadata, health/readiness endpoints, secure baseline headers, and a protected placeholder audit route. | P1 | Partial: boundary implemented; business API pending | Running transaction/audit service, complete OpenAPI document, contract/RPC health checks, and integration tests. |
...
| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| IDX-01 | Implement the event consumer for identity, role, asset, access, transfer, pause, and inherited standard events. | P1 | Partial: canonical projection primitives only; consumer pending | Running RPC consumer, complete event fixtures, and operational backfill. |
| IDX-02 | Use idempotent keys such as chain ID, contract address, transaction hash, log index, and event version. | P0 | Partial: typed event key and duplicate-delivery tests; database constraint pending | Durable unique database constraint and duplicate-delivery integration tests. |
...
| IDX-05 | Detect block gaps, missed logs, duplicate logs, cursor corruption, and incomplete backfills. | P0 | Partial: explicit checkpoint gap detector, contiguous verification, and duplicate tests | Replay command, alerts, RPC fault handling, and recovery report. |
| IDX-06 | Handle chain reorganizations by replaying affected ranges and marking uncertain records. | P0 | Partial: explicit rollback primitive and replay test; canonical reorg worker pending | Reorg simulation against an approved RPC, uncertainty states, and reconciliation evidence. |
...
| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| DOC-01 | Either implement `services/api`, `services/indexer`, `apps/web`, `packages/*`, and Docker Compose or label every related command and configuration value as planned. | P1 | Partial: API boundary and indexer primitives exist; frontend, database, queue, storage, and Compose remain explicitly staged | Clean-checkout command audit. |
...
| DOC-03 | Correct `.github/dependabot.yml` entries for pip ecosystems and `/services/api` if those manifests do not exist. The `/services/api` pip ecosystem is now restored alongside the tracked direct requirements and hashed lock. | P1 | Implemented / monitor | Dependabot configuration matches tracked manifests. |
...
| QA-04 | Add API contract, authentication, authorization, replay, idempotency, and transaction-state tests. | P0 | FastAPI boundary, transaction state-machine tests, and CI report; business API/provider tests remain pending. |
| QA-05 | Add indexer duplicate, gap, retry, reorg, confirmation, backfill, drift, and restore tests. | P0 | Dependency-free duplicate, checkpoint-gap, block-conflict, and rollback tests; integration and restore evidence pending. |
