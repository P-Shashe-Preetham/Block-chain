...
| IDX-08 | Design database migrations, indexes, retention, tenant isolation, encryption, backup, restore, and least-privilege database accounts. | P0 | Partial: ADR 0004 establishes PostgreSQL with SQLAlchemy/Alembic; local models, repository constraints, and initial Alembic upgrade test cover transaction intents, canonical events, checkpoints, and reconciliation findings; PostgreSQL integration, tenant isolation, encryption, backup/restore, and access evidence remain pending | Migration tests, restore drill, schema review, access review. |
