...
| IDX-03 | Implement confirmation/finality policy per network and do not expose unconfirmed ownership as final. | P0 | Partial: projection separates finalized and unfinalized events using an explicit confirmation depth; network-specific finality configuration and durable pending-state behavior remain pending | Confirmation tests, stale/pending state behavior, policy configuration. |
...
| QA-05 | Add indexer duplicate, gap, retry, reorg, confirmation, backfill, drift, and restore tests. | P0 | Dependency-free duplicate, checkpoint-gap, block-conflict, rollback, and explicit confirmation-depth tests; integration, retry, backfill, drift, and restore evidence pending. |
