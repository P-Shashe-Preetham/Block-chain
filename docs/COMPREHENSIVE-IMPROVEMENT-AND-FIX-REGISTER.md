...
| IDX-02 | Use idempotent keys such as chain ID, contract address, transaction hash, log index, and event version. | P0 | Partial: typed event key, SQLAlchemy uniqueness constraint, and repository duplicate/conflict tests; PostgreSQL integration and concurrent-write evidence pending | Durable unique database constraint and duplicate-delivery integration tests. |
