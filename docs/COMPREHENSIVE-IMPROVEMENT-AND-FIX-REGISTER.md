...
| IDX-06 | Handle chain reorganizations by replaying affected ranges and marking uncertain records. | P0 | Partial: rollback now removes affected events from canonical output, retains orphaned records as explicit uncertain events, and clears matching replay identities; canonical reorg worker remains pending | Reorg simulation against an approved RPC, uncertainty states, and reconciliation evidence. |
...
| IDX-08 | Design database migrations, indexes, retention, tenant isolation, encryption, backup, restore, and least-privilege database accounts. | P0 | Partial: ADR 0004 establishes PostgreSQL with SQLAlchemy/Alembic and expand/contract migration direction; schema, constraints, tenant isolation, encryption, backup/restore, and access evidence remain pending | Migration tests, restore drill, schema review, access review. |
