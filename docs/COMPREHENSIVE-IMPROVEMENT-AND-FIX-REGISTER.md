...
| Repository setup documentation was reconciled with implemented Python services, indexer, repository-native web console, a bounded local Compose topology, and a guarded synthetic PostgreSQL backup/restore/reconciliation drill; unavailable production commands remain excluded. | `README.md`, `docs/runbooks/LOCAL-COMPOSE.md`, `docs/runbooks/POSTGRES-BACKUP-RESTORE-DRILL.md`, `docker-compose.yml`, `scripts/drills/postgres_backup_restore_reconcile.sh`, `.github/PULL_REQUEST_TEMPLATE.md`, `.github/dependabot.yml`, `.devcontainer/devcontainer.json` | Full Docker health evidence, production backup/restore/retention posture, and external-gated components remain required. |
...
| SC-ASSET-13 | Add a canonical asset verification CLI or web tool that works independently of the main UI. | P1 | Partial: `scripts/verify_asset.mjs` directly reads supplied RPC/contract evidence for chain ID, deployed-code hash, owner, asset/metadata commitments, lifecycle status, and owner identity state without browser/API/projection trust; it returns non-verified/unavailable on mismatch or failed reads. | Approved network/address/code-hash/finality input, direct-RPC walkthrough, event-history extension, and retained sanitized verification evidence. |
...
| SC-ENG-04 | Strengthen Echidna invariants beyond default-admin and approval checks: role safety, active identities, unique assets, ownership, pause behavior, transfer authority, and event/state consistency. | P0 | Partial: stateful harness covers DID reverse mappings, inactive-role revocation, asset status bounds, terminal transfer denial, access-rule shape, pause/admin persistence, and approval/admin invariants; local pinned 1,000-test campaign passed all eight current properties. | Expanded invariant design/corpus, failure triage, protected CI evidence, and independent review. |
...
| SC-ENG-06 | Add property tests for all error selectors and event arguments, including malformed addresses, zero hashes, duplicate IDs, inactive endpoints, unauthorized callers, and paused state. | P1 | Partial: negative tests now cover empty actions, stale/expired access rules, illegal lifecycle transitions, inactive identities, invalid addresses/hashes, duplicate identity/asset conditions, transfer paths, nonexistent assets, and paused state; full selector/event matrix remains pending | Error/event coverage report and negative-test matrix. |
| SC-ENG-07 | Add invariant checks for total supply, token-owner uniqueness, asset-ID uniqueness, no privileged inactive identities, and no state change on revert. | P0 | Partial: expanded unit negative-path coverage complements the stateful Echidna harness; latest local coverage reports 84.17% Solidity line coverage for the canonical production contract (59.42% total including the uninstrumented harness), while invariant completeness and independent review remain pending | Fuzz and unit evidence linked to invariants. |
...
| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| API-01 | Implement the FastAPI service or remove aspirational API commands until implementation exists. A fail-closed boundary now exists with versioned OpenAPI metadata, health/readiness endpoints, secure baseline headers, RPC chain/code readiness checks, a protected sanitized audit route, and a protected transaction-intent route. | P1 | Partial: boundary, projection reads, and intent recording contract implemented; when an explicit validated database and contract are configured, lazy SQLAlchemy-backed audit and intent adapters are wired by default. Signing/submission and the full state-changing business API remain pending. | Running transaction/audit service, complete OpenAPI document, indexer/database/key-service health checks, and integration tests. |
...
| API-06 | Add idempotency keys for mint, transfer, allocation, access-release, recovery, and upload operations. A deterministic in-memory reference store now rejects conflicting key reuse and returns the original record for identical retries; it is not a multi-instance production store. | P0 | Partial: protected typed transaction-intent route, reference state machine, PostgreSQL unique subject/idempotency constraint, durable retry/conflict repository, explicit SQLAlchemy transaction adapter tests, and lazy API default wiring for an explicit validated database/contract exist. Tenant scope, expiry/retention, and production store policy remain pending. | Duplicate-request tests, durable unique database constraint, retry behavior, expiry/retention, and idempotency storage policy. |
| API-07 | Track transaction state as requested, signed, submitted, pending, confirmed, failed, reverted, replaced, or unknown. A typed state machine now rejects illegal transitions and requires a transaction hash after submission begins. | P0 | Partial: API state machine, protected no-submission intent route, and durable repository transitions now share the closed transition matrix; transaction status is checked in model/migration constraints and durable tests cover illegal transitions, required hashes, terminal states, and retries; signer, chain receipt/event worker, replacement/reorg handling, and UI remain pending | Timeout/replacement tests, receipt/event confirmation, reorg behavior, durable persistence, and UI-visible status. |
...
| API-10 | Publish privacy-safe audit APIs with actor pseudonyms, pagination, authorization, export controls, and redaction rules. | P1 | Partial: injected projection-only `/v1/audit` route has bounded limits, explicit canonical/unfinalized/uncertain filtering, bearer-auth dependency, and sanitized fields; durable reader, actor pseudonyms, cursor pagination, export controls, canonical authorization, and privacy review remain pending | Data-access tests, export samples, privacy review. |
| API-11 | Decide whether the backend uses Python-native SQLAlchemy/Alembic or a TypeScript/Prisma service; remove unused alternatives from the target architecture. | P1 | Partial: ADR 0004 selects the Python-native PostgreSQL/SQLAlchemy/Alembic direction; the exact hash-locked psycopg driver, real disposable PostgreSQL migration/integration test, and sanitized audit-reader boundary now exist, while route/worker wiring remains pending | ADR, dependency policy, migration ownership. |

### 5.2 Indexer and reconciliation

| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| IDX-01 | Implement the event consumer for identity, role, asset, access, transfer, pause, and inherited standard events. | P1 | Partial: read-only RPC scan, strict dependency-free decoder, transaction-scoped persistent sink, and `PersistentConfirmedScanner` now cover the exact compiled `SecureAssetPlatform` application plus inherited ERC-721 and AccessControl event surface; background worker, live approved RPC validation, durable deployment wiring, and operational backfill remain pending | Running RPC consumer, complete event fixtures, and operational backfill. |
| IDX-02 | Use idempotent keys such as chain ID, contract address, transaction hash, log index, and event version. | P0 | Partial: typed event key, strict ABI-decoder fixtures, SQLAlchemy uniqueness constraint, transaction-scoped persistent sink, append-only raw-chain-log identity retention, and repository duplicate/conflict tests; PostgreSQL integration and concurrent-write evidence pending | Durable unique database constraint and duplicate-delivery integration tests. |
...
| IDX-05 | Detect block gaps, missed logs, duplicate logs, cursor corruption, and incomplete backfills. | P0 | Partial: explicit checkpoint gap detector, contiguous verification, duplicate tests, confirmed-range scanner, malformed-response tests, and an atomic one-shot persistent runner that rolls back raw and derived writes on decoder failure | Replay command, alerts, RPC fault handling, and recovery report. |
| IDX-06 | Handle chain reorganizations by replaying affected ranges and marking uncertain records. | P0 | Partial: in-memory and transaction-scoped references remove affected checkpoints from derived output, retain affected event records as explicit uncertain history, and clear only matching replay identities; atomic one-shot scanner rollback is covered, while canonical reorg detection/replay worker remains pending | Reorg simulation against an approved RPC, uncertainty states, and reconciliation evidence. |
| IDX-07 | Periodically reconcile projected ownership, roles, identity status, and asset mappings against canonical contract reads. | P0 | Partial: read-only deterministic drift reporter covers missing, unexpected, and mismatched projection records; a transaction-scoped sink now persists deterministic open findings without repair; live canonical reads, scheduled execution, repair approval, and evidence lifecycle remain pending | Drift test, reconciliation report, repair approval. |
| IDX-08 | Design database migrations, indexes, retention, tenant isolation, encryption, backup, restore, and least-privilege database accounts. | P0 | Partial: ADR 0004 establishes PostgreSQL with SQLAlchemy/Alembic; local models, repository constraints, forward revisions for projection and raw-chain-log tables, migration upgrade tests, a fail-closed engine factory, hash-locked `psycopg[binary]==3.3.4`, and a genuine disposable PostgreSQL migration/integration suite in CI cover transaction intents, canonical events, raw log integrity, checkpoints, and reconciliation findings; the official `DATABASE_URL` contract is used by migrations, SQLite is limited to disposable environments, and non-local PostgreSQL requires secure SSL settings. Tenant isolation, encryption, backup/restore, production client/custody posture, and access evidence remain pending | Migration tests, restore drill, schema review, access review. |
| IDX-09 | Define event schema versioning and backward-compatible handling for contract upgrades or new event fields. | P1 | Partial: event-version participates in durable event identity, generated-ABI topic/name drift is CI-guarded, and forward migration `0003_status_constraints` closes projection status values; formal versioned payload schemas, compatibility migrations, and upgrade fixtures remain pending | Versioned schema, migration fixtures, compatibility tests. |
...
| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| DATA-01 | Implement an approved payload-classification service that rejects raw identity documents, biometrics, secrets, or regulated data unless explicitly approved. | P0 | Partial: `services/storage/classification.py` provides a declared-metadata, bounded prototype gate that permits only public metadata and encrypted asset ciphertext and rejects sensitive/unknown classes; it is not DLP/content inspection and has no privacy-owner approval or real-data integration | Classification rules, rejection tests, privacy owner approval. |
...
The repository now includes an initial Vite/React Evidence Ledger console. It is intentionally read-only, consumes only the typed sanitized audit contract when a public API base URL is configured, stores no browser secret, and contains no signer/wallet/key-release code. Any future web/mobile client must treat the wallet as a signing device, not as proof of organizational identity, and must make transaction scope and authorization state understandable. WCAG 2.2 provides the accessibility baseline for perceivable, operable, understandable, and robust interfaces.[9]

| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| UI-01 | Implement the repository-native Vite/React web console selected by ADR 0009 and remove unsupported client claims from documentation. | P1 | Partial: `apps/web/` provides an Evidence Ledger console with typed sanitized audit fetch, explicit unavailable state, visible control gates, strict TypeScript check, unit/E2E/axe tests, and static build. A measured Ponytail process-reference review removed an unnecessary one-query runtime dependency without changing authority or fail-closed behavior; no approved live API/auth or deployment exists. | Docker-capable running client, approved deployment/API origin, named accessibility review, and governed live data evidence. |
...
| UI-04 | Never display client-provided role labels or cached ownership as authoritative; refresh critical state from API/canonical chain. | P0 | Partial: the console displays no role/owner as authority, labels the browser non-canonical, and reads only a sanitized projection; fresh canonical critical-state UI and stale-read behavior remain pending. | Stale-state and permission-rendering tests. |
| UI-05 | Implement clear pending, confirmed, failed, reverted, replaced, stale, and reconciliation-warning states. | P0 | Partial: the console renders the documented projection statuses and an explicit unavailable state; transaction, reconciliation, and stale-read state UI remain pending. | E2E transaction-state tests. |
...
| UI-07 | Meet WCAG 2.2 baseline: keyboard navigation, focus, labels, headings, contrast, errors, status announcements, timing, responsive layout, and reduced-motion behavior. | P1 | Partial: semantic landmarks, skip link, labels, visible focus treatment, responsive layout, and reduced-motion-aware transitions are implemented; axe and manual keyboard/screen-reader evidence remain pending. | axe or equivalent report plus manual keyboard/screen-reader review. |
...
| UI-09 | Prevent sensitive payloads, keys, tokens, and decrypted content from browser logs, analytics, crash reports, local storage, and URLs. | P0 | Partial: no browser secret persistence, token input, signer, decrypted content, or analytics integration is implemented; browser instrumentation and automated inspection remain pending. | Browser instrumentation review and automated checks. |
| UI-10 | Add user education for token ownership versus legal title, access permission versus decryption, irreversible signatures, and recovery limits. | P1 | Partial: the console labels non-canonical/unavailable boundaries and no-submission scope; full user education and usability review remain pending. | Content review and usability test. |
...
| DOC-12 | Keep `THIRD-PARTY-NOTICES.md`, reference catalog, integration matrix, and license boundaries current; do not imply that reference repositories are integrated runtime code unless they are actually adopted. | P1 | Partial: reference validator preserves all 96 sources and the ledger now defines a no-inference activation protocol; exact consulted revisions, accountable owners, license decisions, and notices remain required before deeper adoption | Reference validator, per-source review records, and quarterly revalidation. |
| DOC-13 | Add an explicit data dictionary for every on-chain field, event field, off-chain record, identifier, hash, CID, and audit field. | P1 | Partial: `docs/FINAL-PROJECT-DATA-DICTIONARY.md` now maps implemented contract fields/events, raw/projection/intent records, API redaction, storage-policy metadata, identifiers/statuses, and prohibited placements. Security/privacy/data-owner review and any real CID/tenant/retention schema remain pending. | Reviewed data dictionary, approved sensitive-data policy, and synchronized implementation updates. |
| DOC-14 | Add versioned migration notes whenever a contract event, role, metadata commitment, environment variable, API route, or storage schema changes. | P1 | Partial: `docs/FINAL-PROJECT-MIGRATION-NOTES-TEMPLATE.md` now requires canonical/version, compatibility, replay, redaction, failure, test, and release evidence treatment. No completed production-style migration sample or external change approval exists. | Completed migration notes/rehearsals for each applicable change and actual external approvals where required. |
...
| GOV-05 | Record risk owner, mitigation, expiry/review date, residual risk, and approval authority for every accepted risk. | P1 | Partial: governance baseline records current risk families, controls, and closure evidence; named accountable owners, expiry/review dates, residual-risk acceptance, and approval authority remain unassigned external governance work | Risk register with owner, expiry, approval, and review records. |
...
| GOV-07 | Define release authority, deployment authority, key-custody authority, incident authority, and emergency authority separately. | P0 | Partial: final-project governance baseline separates product, contract/deployment, identity, storage/key, privacy/legal, incident, review, and operations responsibilities; real role appointments, access review, and break-glass evidence remain external blockers | Approved RACI, access review, and break-glass log. |
