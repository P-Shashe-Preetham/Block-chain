...
| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| API-01 | Implement the FastAPI service or remove aspirational API commands until implementation exists. A fail-closed boundary now exists with versioned OpenAPI metadata, health/readiness endpoints, secure baseline headers, RPC chain/code readiness checks, and a protected audit route. | P1 | Partial: boundary and sanitized read-only audit projection contract implemented; state-changing business API pending | Running transaction/audit service, complete OpenAPI document, indexer/database/key-service health checks, and integration tests. |
...
| API-06 | Add idempotency keys for mint, transfer, allocation, access-release, recovery, and upload operations. A deterministic in-memory reference store now rejects conflicting key reuse and returns the original record for identical retries; it is not a multi-instance production store. | P0 | Partial: reference state machine, PostgreSQL unique subject/idempotency constraint, durable retry/conflict repository, and explicit SQLAlchemy transaction adapter tests exist; API route integration, tenant scope, expiry/retention, and durable production store policy remain pending | Duplicate-request tests, durable unique database constraint, retry behavior, expiry/retention, and idempotency storage policy. |
| API-07 | Track transaction state as requested, signed, submitted, pending, confirmed, failed, reverted, replaced, or unknown. A typed state machine now rejects illegal transitions and requires a transaction hash after submission begins. | P0 | Partial: API state machine and durable repository transitions now share the closed transition matrix; transaction status is checked in model/migration constraints and durable tests cover illegal transitions, required hashes, terminal states, and retries; chain receipt/event worker and UI remain pending | Timeout/replacement tests, receipt/event confirmation, reorg behavior, durable persistence, and UI-visible status. |
