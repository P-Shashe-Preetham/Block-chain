...
| IDX-09 | Define event schema versioning and backward-compatible handling for contract upgrades or new event fields. | P1 | Partial: event-version participates in durable event identity, and a build-time guard compares decoder event names/topic hashes with the generated canonical ABI; formal versioned payload schemas, compatibility migrations, and upgrade fixtures remain pending | Versioned schema, migration fixtures, compatibility tests. |
