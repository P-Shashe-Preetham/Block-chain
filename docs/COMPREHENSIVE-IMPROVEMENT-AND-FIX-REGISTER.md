...
| ID | Improvement or fix | Priority | Current state | Acceptance evidence |
|---|---|---:|---|---|
| IDX-01 | Implement the event consumer for identity, role, asset, access, transfer, pause, and inherited standard events. | P1 | Partial: read-only RPC scan and decoder callback baseline implemented; contract ABI decoder and complete event consumer pending | Running RPC consumer, complete event fixtures, and operational backfill. |
...
| QA-04 | Add API contract, authentication, authorization, replay, idempotency, and transaction-state tests. | P0 | FastAPI boundary, explicit-settings HTTPX tests, bounded rate-limit tests, transaction state-machine tests, and CI report; business API/provider tests remain pending. |
| QA-05 | Add indexer duplicate, gap, retry, reorg, confirmation, backfill, drift, and restore tests. | P0 | Dependency-free duplicate, checkpoint-gap, block-conflict, rollback, confirmation-depth, RPC failover, malformed-response, and deterministic drift tests; integration, backfill, repair, and restore evidence pending. |
