...
| API/service boundary | A fail-closed FastAPI boundary validates configuration, request IDs, CORS, security headers, RPC chain/code readiness, bearer-auth presence, and typed local transaction/indexer reference behavior; business transaction/audit routes are not implemented | **Partially compliant** | Approved OIDC/JWKS or wallet verifier, route authorization, durable transaction state/idempotency, rate limits, indexer, and database integration |
