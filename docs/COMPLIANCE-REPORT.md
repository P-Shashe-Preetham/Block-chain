...
|---|---|---:|---|
| SSI/DID-based identity | ECDSA/secp256k1 wallet authentication and DID-hash lifecycle are implemented; this remains a blockchain-backed DID registry/reference, not complete SSI | **Partially compliant** | Approved DID method, DID-document/verification-method design, and identity assurance review |
...
| NFT asset ledger | ERC-721 unique assets, unique organizational asset IDs, fixed-size metadata hashes, manager-only transfers, and duplicate tests | **Compliant for MVP** | Physical-asset registry, standard-selection approval, independent review, and deployment evidence |
...
| Immutable audit trail | Structured contract events and non-reverting access decisions are implemented; indexer and reconciliation are absent | **Partially compliant** | Event schema publication, indexer, transaction-status pipeline, independent verification, and audit export |
...
| Duplicate physical assets | Unique organizational `assetId` mapping rejects duplicate asset registration | Contract, test, traceability matrix | **Fixed for MVP** |
